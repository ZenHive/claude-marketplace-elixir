...
test_hook_json \
  ...
  "{\"tool_input\":{\"file_path\":\"$REPO_ROOT/test/plugins/ash/postedit-test/lib/postedit_test/accounts/user.ex\"},\"cwd\":\"$REPO_ROOT/test/plugins/ash/postedit-test\"}" \
  0 \
  ".hookSpecificOutput | has(\"additionalContext\")"
test_hook_json \
  "Post-edit check: Works on .exs files" \
  "plugins/ash/scripts/post-edit-check.sh" \
  "{\"tool_input\":{\"file_path\":\"$REPO_ROOT/test/plugins/ash/postedit-test/test/test_helper.exs\"},\"cwd\":\"$REPO_ROOT/test/plugins/ash/postedit-test\"}" \
  0 \
...
test_hook_json \
  ...
  "{\"tool_input\":{\"command\":\"git commit -m 'test'\"},\"cwd\":\"$REPO_ROOT/test/plugins/ash/precommit-test\"}" \
  0 \
...
test_hook_json \
  ...
  "plugins/ash/scripts/pre-commit-check.sh" \
  "{\"tool_input\":{\"command\":\"git -C $REPO_ROOT/test/plugins/ash/precommit-test commit -m 'test'\"},\"cwd\":\"$REPO_ROOT\"}" \
  0 \
...
test_hook_json \
  ...
  "plugins/ash/scripts/pre-commit-check.sh" \
  "{\"tool_input\":{\"command\":\"git -C /nonexistent/path commit -m 'test'\"},\"cwd\":\"$REPO_ROOT/test/plugins/ash/precommit-test\"}" \
  0 \
