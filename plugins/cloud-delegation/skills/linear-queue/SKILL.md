...
| 6. Audit (deferred) | User invokes `Skill(audit-review) <range>` after SessionStart hook flags unaudited tail; skill writes `.audit/<sha>.md` per merge SHA in range | `save_comment(issueId, "Audited at <audit-sha>: <one-line summary>")` (optional — only post if findings, otherwise the audit commit + `.audit/<sha>.md` is the trail) | One line |
...
**Identical:** code-only PRs, plan-shaped specs, deferred post-merge `audit(...)` commit on the repo's default branch (user-invoked `Skill(audit-review)` over a range), draft-PR handling, bot ensemble integration in commit-review.
...
- `staged-review:audit-review` skill — deferred post-merge hygiene + bookkeeping; SessionStart hook surfaces unaudited tails, user invokes over a range
